 ...
 * a second parser. GitHub shallow fetch is not this lane — `github-install-source.ts`
 * only resolves repository URLs, and this file does not download a tarball.
 *
 * Re-bind regeneration is not hooked. After `bindConnectedTarget` succeeds in
 * `routes/targets.ts` (POST handler, after the snapshot bind), call
 * `acquireTargetContext`. This module is the on-demand path.
 *
...
export async function acquireTargetContext(
  ...
  if (target.kind !== 'local-folder') {
    return {
      ...
      status: 400,
      code: 'BAD_REQUEST',
      message: 'target context acquisition supports local-folder targets only',
    };
  }
